...
# 0) CONFIG
# =========================================================
# Cropped ROI root. Expected structure:
# ROOT/
#   l/
#   lf1/
#   lf2/
...
# Change these for each ablation run.
# Examples:
#   INPUT_GROUPS = ["l"]          -> large focus baseline: l -> s
#   INPUT_GROUPS = ["lf1"]        -> single speckle: lf1 -> s
#   INPUT_GROUPS = ["lf1", "lf1"] -> duplicate 2ch control: [lf1, lf1] -> s
EXPERIMENT_NAME = "lf1_lf2_to_s_train100_maskp95_softloss"
INPUT_GROUPS = ["lf1", "lf2"]
GT_GROUP = "s"

# Use only a few projection angles as independent training scenes.
# These are NOT used as multi-angle input channels. Each sample is still same-angle input -> same-angle GT.
TRAIN_INDICES = list(range(0, 360, 6))      # 60张
VAL_INDICES   = [366, 390, 414, 438, 462]  # 5张

# Cropped RAW size. ImageJ showed Width x Height = 1984 x 1856.
# NumPy uses H, W.
H, W = 1856, 1984
RAW_DTYPE = np.uint16
...
# Flat-field is disabled by default because the new ablation folders may not include flat frames.
USE_FLAT = False
FLAT_GROUPS = {
    "l": "flat_l",
    "lf1": "flat_lf1",
...
# Keep the same semantic mask sampling logic as your previous script.
MAG_THR = 0.12
MASK_DILATE = 15
...
def masked_edge(edge_loss_fn, pred, target, mask):
    w = mask.float().clamp(0, 1)
    return edge_loss_fn(pred.float() * w, target.float() * w)
...
def load_raw_scene(idx: int, flat_cache: Dict[str, np.ndarray]) -> RawScene:
...
class MultiScenePatchDataset(Dataset):
    """
    Multiple projection scenes.
    Each sample:
      1) randomly choose one projection angle from scenes
      2) sample a semantic-mask-centered patch using the same logic as the previous script
      3) return x=[C,P,P], y=[1,P,P], mask=[1,P,P]
    """
...
def main():
    ...
    print(f"[Split] train={TRAIN_INDICES}, val={VAL_INDICES}")
    ...
    train_raw = [load_raw_scene(idx, flat_cache) for idx in TRAIN_INDICES]
    print("[Load] loading val scenes...")
    val_raw = [load_raw_scene(idx, flat_cache) for idx in VAL_INDICES]

    # Compute normalization stats from TRAIN scenes only.
    norm_pool = []
    for sc in train_raw:
    ...
    for sc in val_scenes:
        coord_n = 0 if sc.coords is None else len(sc.coords)
        print(f"[Scene val]   {sc.name}: mask_mean={sc.mask01.mean():.5f}, coords={coord_n}")

    # Save quick previews for sanity check.
    preview_dir = os.path.join(SAVE_DIR, "previews")
    os.makedirs(preview_dir, exist_ok=True)

    preview_scenes = train_scenes[:3] + val_scenes[:2]
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Save a random patch preview from the first train scene.
        if epoch == 1 or epoch % 2 == 0 or epoch == EPOCHS:
            ...
            save_u8(os.path.join(SAVE_DIR, "patch_gt_center.png"), sc.gt01[cy-half:cy+half, cx-half:cx+half])
